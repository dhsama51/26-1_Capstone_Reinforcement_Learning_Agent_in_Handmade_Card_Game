...
def _strip_private_config(config: Dict[str, Any]) -> Dict[str, Any]:
    return {str(key): value for key, value in dict(config).items() if not str(key).startswith("_")}
...
def _worker_loop(conn, card_data_path):
    ...
    try:
        ...
        while True:
            cmd, args = conn.recv()
            if cmd == "init_game":
                snapshot = session.init_game(**_strip_private_config(args or {}))
                conn.send(snapshot)
...
class VectorSeaEngineEnv:
    ...
    Backends:
    - isolated (default): spawned worker processes with PythonNetSession reuse
    - local: keep sessions in current process, optional thread pool
    ...
    def reset_timing_stats(self) -> None:
        if self.backend == "isolated" and self._isolated_env is not None:
            self._isolated_env.reset_timing_stats()

    def get_timing_stats(self) -> Dict[str, Any]:
        if self.backend == "isolated" and self._isolated_env is not None:
            return self._isolated_env.get_timing_stats()
        return {
            "last_step_wait_sec": 0.0,
            "total_step_wait_sec": 0.0,
            "total_init_wait_sec": 0.0,
            "step_wait_calls": 0,
            "step_wait_results": 0,
            "max_step_wait_sec": 0.0,
        }
    ...
    def init_games(self, configs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        if self.backend == "process":
            ...
            for i in range(limit):
                self.pipes[i].send(("init_game", _strip_private_config(configs[i])))
        ...
        if self._executor is None or limit <= 1:
            results = [self._sessions[i].init_game(**_strip_private_config(configs[i])) for i in range(limit)]
            return results

        futures = [self._executor.submit(self._sessions[i].init_game, **_strip_private_config(configs[i])) for i in range(limit)]
    ...
    def step_async(self, cmds: List[Optional[tuple]]):
        ...
        for i in range(limit):
            cmd = cmds[i]
            if cmd is not None and cmd[0] != "configure_worker":
