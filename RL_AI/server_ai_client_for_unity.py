...
def _normalize_board_card(raw_card: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "is_attacked": bool(raw_card.get("isAttacked", False)),
        "pos_x": pos_x,
        "pos_y": pos_y,
        "atk": atk,
        "effective_atk": atk,
        ...
        "max_hp": max_hp,
        "buffs": buffs,
        "statuses": _extract_statuses(buffs),
...
def _extract_statuses(raw_buffs: Any) -> list[Dict[str, Any]]:
    if isinstance(raw_buffs, dict):
        return [
            {"type": str(key), "value": _safe_int(val, 0), "remaining_turns": 1}
            for key, val in raw_buffs.items()
        ]

    statuses: list[Dict[str, Any]] = []
    for buff in list(raw_buffs or []):
        if not isinstance(buff, dict):
            continue
        buff_type = str(buff.get("id", buff.get("Id", buff.get("type", buff.get("Type", ""))))).strip()
        if not buff_type:
            continue
        buff_value = buff.get("Amount", buff.get("amount", buff.get("Value", 0)))
        statuses.append(
            {
                "type": buff_type,
                "value": _safe_int(buff_value, 0),
                "remaining_turns": 1,
            }
        )
    return statuses
...
def server_json_to_snapshot(packet_json: Dict[str, Any]) -> Dict[str, Any]:
    ...
    reward = _safe_float(data.get("Reward", packet_json.get("Reward", 0.0)), 0.0)

    players = [
        {
            "id": "P1",
            "server_id": str(p1_raw.get("Id", "Player1")),
            "hand_count": _safe_int(p1_raw.get("HandCount", len(p1_hand_uids)), len(p1_hand_uids)),
            "deck_count": _safe_int(p1_raw.get("DeckCount", len(p1_deck_uids)), len(p1_deck_uids)),
            "trash_count": _safe_int(p1_raw.get("TrashCount", len(p1_trash_uids)), len(p1_trash_uids)),
        ...
        },
        {
            ...
            "hand_count": _safe_int(p2_raw.get("HandCount", len(p2_hand_uids)), len(p2_hand_uids)),
            "deck_count": _safe_int(p2_raw.get("DeckCount", len(p2_deck_uids)), len(p2_deck_uids)),
            "trash_count": _safe_int(p2_raw.get("TrashCount", len(p2_trash_uids)), len(p2_trash_uids)),
...
class ServerAiClient:
    ...
    def run(self) -> None:
        with socket.create_connection((self.host, self.port)) as sock:
            ...
            _DECKS = {
                "orange":    '["Or_L", "Or_B", "Or_N", "Or_R", "Or_P", "Or_P", "Or_P"]',
                "charlotte": '["Cl_L", "Cl_B", "Cl_N", "Cl_R", "Cl_P", "Cl_P", "Cl_P"]',
            }
