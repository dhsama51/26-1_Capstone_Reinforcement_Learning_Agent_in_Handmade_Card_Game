...
def test_server_json_to_snapshot_normalizes_server_payload():
    packet_json = {
        "Data": {
            "Player1": {"Id": "Player1", "Hand": ["C000"], "Deck": ["C001"], "Trash": [], "HandCount": 1, "DeckCount": 1, "TrashCount": 0},
            "Player2": {"Id": "Player2", "Hand": ["C002"], "Deck": ["C003"], "Trash": [], "HandCount": 1, "DeckCount": 1, "TrashCount": 0},
            "Board": [
                {"Uid": "C000", "Id": "Or_L", "Owner": "Player1", "isPlaced": True, "isMoved": False, "isAttacked": True, "X": 0, "Y": 2, "Atk": 3, "Hp": 9, "MaxHp": 9, "Buff": [{"id": "MoveLock", "Amount": 1}]},
    ...
    assert snapshot["board"][0]["is_attacked"] is True
    assert snapshot["board"][0]["statuses"][0]["type"] == "MoveLock"
    assert snapshot["players"][0]["hand"][0]["uid"] == "C000"
    assert snapshot["players"][0]["hand_count"] == 1
    assert snapshot["players"][0]["deck_count"] == 1
    assert snapshot["players"][0]["trash_count"] == 0
